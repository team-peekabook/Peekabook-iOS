...
final class BookShelfAPI {
    ...
    private (set) var editBookData: GeneralResponse<BlankData>?
    ...
    func editMyBookInfo(id: Int, param: EditBookRequest, completion: @escaping (GeneralResponse<BlankData>?) -> Void) {
        bookShelfProvider.request(.editBookInfo(id: id, param: param)) { [self] (result) in
            switch result {
            case .success(let response):
                do {
                    self.editBookData = try response.map(GeneralResponse<BlankData>.self)
